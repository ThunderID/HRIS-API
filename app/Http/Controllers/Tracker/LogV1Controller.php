...
/**
 * Handle Tracker version 1.* inputs
 * 
 * @author cmooy
 */
class LogV1Controller extends Controller 
{
	/**
	 * Store logs
	 *
	...
	{		
		...
		if(!$results)
		{
			$filename                       	= storage_path().'/logs/appaccess.log';
			$fh                             	= fopen($filename, 'a+'); 
			$template 							= date('Y-m-d H:i:s : Upaya Masuk '.$_SERVER['REMOTE_ADDR'])."\n";
	        fwrite($fh, $template); 
	        fclose($fh);
